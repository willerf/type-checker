...
std::vector<std::shared_ptr<ASTNode>> extract_args(ParseNode root) {
    ...
    if (prod == std::vector<State> {NonTerminal::args, NonTerminal::expr}) {
        ...
        auto child_result = extract_args(args);
...
std::vector<std::shared_ptr<ASTNode>> extract_optargs(ParseNode root) {
