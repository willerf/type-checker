...
std::shared_ptr<LType> make_lt(LType ltype) {
    return std::make_shared<LType>(ltype);
}
std::shared_ptr<LType> make_lt(LTypeClass ltype) {
    return std::make_shared<LType>(std::vector<LTypeClass>({ltype}));
}
...
std::shared_ptr<LType> TypeVisitor::visit(std::shared_ptr<BinaryExprNode> node) {
    ...
    std::shared_ptr<LType> out_type = nullptr;
    switch (node->op) {
        case BinaryOp::OR:
        case BinaryOp::AND:
